...
# Sección "Inactivos Agenda" añadida
elif seccion == "Inactivos Agenda":
    st.header("📋 Inactivos Agenda")
    
    # Cargar el archivo Excel
    archivo_inactivos = st.file_uploader("📁 Subí tu archivo con las dos hojas:", type=["xlsx", "xls"], key="inactivos_agenda")
    
    if archivo_inactivos:
        df = pd.read_excel(archivo_inactivos, sheet_name="Hoja 1")
        df2 = pd.read_excel(archivo_inactivos, sheet_name="Hoja 2")

        # Preparar el dataframe de la Hoja 1
        df = df.rename(columns={"Nombre": "Jugador"})
        df2 = df2.rename(columns={"Al usuario": "Jugador"})

        # Filtrar los jugadores que están en ambas hojas
        jugadores_comunes = pd.merge(df, df2, on="Jugador", how="inner")

        # Preparar el dataframe final
        resumen = []

        for jugador in jugadores_comunes["Jugador"].unique():
            historial = df2[df2["Jugador"] == jugador].sort_values("Fecha")
            cargas = historial[historial["Tipo"] == "in"]
        ...
        df_registro.to_excel("inactivos_agenda.xlsx", index=False)
        with open("inactivos_agenda.xlsx", "rb") as f:
            st.download_button("📥 Descargar Excel", f, file_name="inactivos_agenda.xlsx")
