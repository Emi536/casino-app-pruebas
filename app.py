...
archivo = st.file_uploader("📁 Subí tu archivo de reporte:", type=["xlsx", "xls", "csv"])
...
if archivo:
    ...
    # DEBUG: mostrar columnas luego del renombrado
    st.info("📋 Columnas luego del renombrado:")
    st.write(df.columns.tolist())
    ...
    st.info(f"🔎 Filas después del filtro: {df_filtrado.shape[0]}")
    if df_filtrado.empty:
        st.warning("⚠️ No hay datos en el rango de fecha y hora seleccionado.")
        st.stop()
