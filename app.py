...
if  auth_status is False:
    ...
    #SECCIÓN ATENEA
        ...
        try:
            hoja_atenea = sh.worksheet("registro_atenea")
            data_atenea = hoja_atenea.get_all_records()
            df_historial = pd.DataFrame(data_atenea)
        except:
            hoja_atenea = sh.add_worksheet(title="registro_atenea", rows="1000", cols="20")
            df_historial = pd.DataFrame()
        ...
        if not df_historial.empty:
            ...
            col1, col2 = st.columns(2)
            with col1:
                filtro_desde = st.date_input("📆 Desde", value=df_historial["Fecha"].min().date(), key="desde_historial_atenea")
            with col2:
                filtro_hasta = st.date_input("📆 Hasta", value=df_historial["Fecha"].max().date(), key="hasta_historial_atenea")
            ...
            resumen_path = "resumen_atenea_cache.pkl"
            hash_path = "resumen_atenea_hash.txt"
            ...
            try:
                hoja_princi = sh.worksheet("princi_atenea")
                data_princi = hoja_princi.get_all_values()
                df_princi = pd.DataFrame(data_princi[1:], columns=data_princi[0])
            ...
            try:
                # 🧩 COMPLETAR TIPO DE BONO desde hoja 'registro_bono_eros'
                hoja_users = sh.worksheet("registro_bono_atenea")
                raw_data_users = hoja_users.get_all_values()
                headers_users = raw_data_users[0]
                rows_users = raw_data_users[1:]
                df_users = pd.DataFrame(rows_users, columns=headers_users)
        ...
        # 🔵 Tabla Bono Atenea desde hojas "registro_users" y "bonos_ofrecidos"
        try:
            # Leer hoja principal ignorando posibles conflictos de encabezado
            hoja_registro = sh.worksheet("registro_bono_atenea")
            raw_data = hoja_registro.get_all_values()
            headers = raw_data[0]
            ...
            # Leer hoja con categorías de bonos
            hoja_bonos = sh.worksheet("bonos_ofrecidos_atenea")
            raw_data_bonos = hoja_bonos.get_all_values()
            headers_bonos = raw_data_bonos[0]
            ...
            # Mostrar en la app
            st.subheader("🎁 Tabla Bono - Atenea")
            st.dataframe(df_bono)
